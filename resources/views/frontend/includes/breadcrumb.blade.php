	...
	<div class="container">
		...
		<div class="left-column content-column">
			<div class="inner-column color-white">
				<h1 class="breadcrumbs-heading" title="{{$datas['full_title'] ? $datas['full_title']: $datas['title']}}">{{$datas['title']}}</h1>
				<ul class="flex">
                    @foreach ($datas['paths'] as $route=>$path)
                        <li title="{{$path}}"><a href='{{($route != 'javascript:void(0)') ? route($route) : $route}}'>{{$path}}</a></li>
                        <li><i class="fa-solid fa-angle-right"></i></li>
                    @endforeach
					<li title="{{$datas['full_title'] ? $datas['full_title']: $datas['title']}}">{{$datas['title']}}</li>
				</ul>
			</div>
		</div>
	</div>
