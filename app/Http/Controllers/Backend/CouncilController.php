...
class CouncilController extends Controller
{
    ...
    {
        $s['council_members'] = CouncilMember::with(['council', 'member'])->whereHas('member', function ($q) {
            $q->whereNull('deleted_at'); // Only include non-deleted members
        })->where('deleted_at', null)->where('council_id', $id)->orderBy('order_key', 'ASC')->get();
